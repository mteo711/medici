<?php
   ...
   //schede
    $obj = new schede();
    $records = $obj->fetchRecord(array("idcaso"=>$idcaso));

    $typo = $records['tipologia'];
    $_SESSION["case_id"] = $records['id'];
    $_SESSION["id_caso"] = $records['idcaso'];

    //log
    $insert_log = array();
    $obja = new log_activities();
    $insert_log["utente"] = $_SESSION["username"];
    $insert_log["oggetto"] = "Visualizzazione";
    $insert_log["operazione"] = "Apertura scheda conclusa";
    $insert_log["schede_id"] = $_SESSION["case_id"];
    $insert_log["id"] = $idcaso;
    $obja->insert($insert_log);
    
    if($records['utenti_centri_id'] == $_SESSION['centri_id']){
        $_SESSION["stato"] = 'chiusa_usr';
    }
    else{
        $_SESSION["stato"] = 'chiusa';
    }
