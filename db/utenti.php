...
/*
CREATE  TABLE IF NOT EXISTS `sids_mf_db`.`utenti` (
  ...
  INDEX `fk_utenti_centri` (`centri_id` ASC) ,
  CONSTRAINT `fk_utenti_centri`
    FOREIGN KEY (`centri_id` )
    REFERENCES `sids_mf_db`.`centri` (`id` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)  
*/

class utenti extends Database
{
 private  $columns = array('centri_id' => 'int',
				'username'  => 'string',
				'password'  => 'string',
				'otp'  => 'string',
				'nome'  => 'string',
...
public function saveSecret($username, $secret) {
  $set_val_cols = array('otp' => $secret);
  $cod_val_cols = array('username' => $username);
