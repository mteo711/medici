...
/*
CREATE  TABLE IF NOT EXISTS `sids_mf_db`.`fumo_passivo` (
  ...
  `luogoA` SET('casa','lavoro') NULL DEFAULT NULL ,
  `durante_gravidanza` ENUM('Y','N') NULL DEFAULT NULL ,
  `fino_a_settimana` INT UNSIGNED NULL DEFAULT NULL ,
  `luogoB` SET('casa','lavoro') NULL DEFAULT NULL ,
  `post_parto` ENUM('Y','N') NULL DEFAULT NULL ,
  `fino_a_eta_bambino_in_giorni` INT UNSIGNED NULL DEFAULT NULL ,
  `luogoC` SET('casa','lavoro') NULL DEFAULT NULL ,
  PRIMARY KEY (`madre_schede_id`) ,
  INDEX `fk_fumo_passivo_madre` (`madre_schede_id` ASC) ,
  CONSTRAINT `fk_fumo_passivo_madre`
...
class fumo_passivo extends Database
{
 ...
 private  $columns = array('madre_schede_id' => 'int',
				...
				'luogoB'  => 'set',
				'post_parto'  => 'bool',
				'fino_a_eta_bambino_in_giorni'  => 'int',
				'luogoC'  => 'set',
                'conclusa' => 'bool'
				);
...
{	
  ...
  foreach ($val_cols as $col => $value)
  {
	 ...
	 // verifico i valori di tipo set
		if ($col=='luogoA' || $col=='luogoB' || $col=='luogoC')
		{ if (!$this->isInSet($value, array('casa','lavoro'))) 
			{ $this->error_status=8;
			  $error=1;
			  break;
			}
